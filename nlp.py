...
def qa(docs, query, return_score=False, return_all=False, return_source=False, sort=False):
    ...
    Parameters
    ----------
    docs,query,return_score,return_all,return_source,sort
...
def numeric_helper(q,key):
    return True if key in q else False

def format_subques(sub_ques,col,typ,val):
    if typ == 'Integer':
        sub_ques = sub_ques.format(col,val)
        sub_ques = sub_ques.replace("'","")
    else:
        sub_ques = sub_ques.format(col,val)
    return sub_ques
...
class Nlp:
    ...
    def slot_fill(self,df, q):
        ...
        for col in schema['columns']:
            ...
            if _is_numeric(coltype):
                handler = numeric_helper(q,keyword)
                if handler:
                    colquery="how much {}".format(keyword)
                else:
                    continue
            else:
            ...
            slots.append((colname, coltype, replace_space(val), score, start_idx, end_idx))
        ...
        for s in slots:
            if s[1] == "FuzzyString":
                ...
                numeric_handling += 'numeric'
            else:
                val = get(s[1])().adapt(s[2])
            if val is not None:
                if numeric_handling == 'numeric':
                    ret.append((s[0], s[1], val, s[3]))
                else:
                    if len(val) == 1:
                        ret.append((s[0], s[1], val[0], s[3]))
                    #ret.append((s[0], s[1], val, s[3]))
                    if len(val) != 1:
                        for i in val:
                            ret.append((s[0],s[1],i,s[3]))
        return ret
    ...
    def type_tag(self,slots):
        types = {}
        type_tag_schema = self.schema
        for i in slots['slots']:
            if i not in types:
                for _ in type_tag_schema['columns']:
                    if _['name'] == i:
                        types[i] = _['type']
        return types
    ...
    def get_sql_query(self,df,q,distinct):
        ...
        if question not in "SELECT {} FROM {}":
            if question == 'SELECT MIN({}) FROM {}' or question == 'SELECT MAX({}) FROM {}':
                if len(unknown_slots['slots']) > 1:
                    get_types = self.type_tag(unknown_slots)
                    main_slot = get_key(get_types, 'Integer')
                    slots = get_key(get_types, 'FuzzyString')
                    if question == 'SELECT MIN({}) FROM {}':
                        question = 'SELECT MIN({}),{} FROM {}'
                    else:
                        question = 'SELECT MAX({}),{} FROM {}'
                else:
        ...
        if question == 'SELECT MIN({}),{} FROM {}' or question == 'SELECT MAX({}),{} FROM {}':
            #print(question)
            question = question.format(main_slot,slots,schema["name"].lower())
        else:
        ...
        for i,s in enumerate(sf):
            ...
            subq = format_subques(sub_clause,col,s[1],val)

            if condflag:
                subq=subq.replace('=','')
                subq=subq.replace("'","")
            conditionalcol.append(col)
