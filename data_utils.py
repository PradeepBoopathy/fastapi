...
class data_utils:
    ...
    def get_schema_for_csv(self,df_path):
        ...
        columns=data.columns.tolist() 
        columns=[self.rename(i) for i in columns]              #self.rename
    ...
    def alter_json(self,value):
        values = {}
        for _, k in value.items():
            singlecomma, multicomma, wcomma = [], [], []
            for i in k:
                if ',' in i:
                    length = len(i)
                    checkdual = i.replace(',', '')
                    if len(checkdual) == length - 1:
                        singlecomma.append(i)
                    else:
                        multicomma.append(i)
                else:
                    wcomma.append(i)
            values[_] = wcomma + singlecomma + multicomma
        return values
